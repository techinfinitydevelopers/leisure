...
## 3D models (GLB) — current state (2026-08-26)
- **All six products now have per-colour GLBs wired** via `model:` (fallback) + `colorModels:` (keyed by colour `id`) in `product-experience.ts`. The old single-model files (`drift-model.glb`, `edge-model.glb`, `legend-model.glb`) were **deleted** by the user and replaced with `{Product}-{Colour}.glb` sets. `skipPlaneAnimation` is gone from Core/Elevate.
- **`colorModels` keys must match the RUNTIME colour id, which is `slugify(colour.name)` from the DB/Shopify — NOT the static `colors[]` list in this file.** Two real bugs came from this (fixed 2026-08-26): DRIFT's live second colourway is "White" → `white`, but the static list says "Grey" → `grey`; DOMINATOR's is "Light Grey" → `light-grey` but the static list says `grey`. Both now map extra keys to the same GLB. **Verify with `curl <page> | grep aria-label` on the swatches before trusting the static list.** A miss here silently falls back to `model` instead of erroring.
- **DOMINATOR is the user's reference product — do not touch its config without being asked.** (Explicit instruction 2026-08-26; its `light-grey` key was reverted for this reason and still relies on the accidental `model`-fallback.)
- `ProductModel.tsx` normalises scale by the **largest** bbox dimension. That makes long/thin products render short and wide: EDGE (500×94×93 mm bar) came out **35% wider and 60% shorter** on screen than DOMINATOR at the same settings. Fix was `modelScale: 1.35 → 1.0`, which lands its width exactly on DOMINATOR's in every section.
- **Useful technique**: on-screen footprint can be computed exactly instead of eyeballed — parse the GLB bbox (`npx @gltf-transform/cli inspect`, or read the JSON chunk; note quantised meshes need the CLI, raw accessor min/max are int16), then project through the page camera (`fov 40 @ z=6` → viewport 4.37 × (4.37·aspect) world units). Used this to tune EDGE when screenshots were unusable.
- `RevolveShowcase.tsx` (home page) also loads a Legend GLB and has its **own** `BASE_RY = 0`, separate from `product-experience.ts`'s `modelBaseRy`. Both were tuned for the now-deleted `legend-model.glb`; RevolveShowcase now points at `Legend-Black.glb`. **The two constants disagree for what is now the same file — at least one facing is wrong, unverified.**

## EDGE product page — status (2026-08-26: bespoke layer REMOVED)
- EDGE no longer has a bespoke page. The `if (slug === "edge")` branch in `app/product/[slug]/page.tsx` is gone and **every product renders through the one shared `ProductExperience`**. `EdgeExperience.tsx`, `EdgeSections.tsx`, `edge-experience.css` were **deleted** (recover from git if the cinema-mode sections are ever wanted back).
- Deliberately accepted losses: the orange `--edge-accent` palette, the scroll-driven colourway switching (`EdgeColoursStack` was the only component anywhere that called `onColor()` from a ScrollTrigger — colour is now hero-swatch-only), and the explicit `Buy EDGE — ₹8,900` CTA button.
- EDGE had no `features` / `deepDives` / `faq` copy (the bespoke layout never had those sections). Copy was **derived from EDGE's own specs** and added — flagged in-file as needing a copy review before shipping.
- `perspective.specLayout: "explorer"` was removed from EDGE — **required**, not cosmetic: `SpecExplorer` deliberately never hides the roaming model (it sits at z-index 40 over the canvas's 30), so Specifications had to fall back to the default `SpecsSection` to get the model-hidden treatment.

## Model visibility per section — the "omit the field" convention
...
- **Section parity confirmed empirically**: `curl <page> | grep -oE 'class="(hero|featureseq|...)'` across all six — identical order, only DOMINATOR's `seqreveal` differs (expected).
- **Per-colour GLBs wired for all six.** Found and fixed two silent `colorModels` key mismatches (DRIFT `white`, DOMINATOR `light-grey`) — see the GLB section above for the rule.
- **EDGE model position/scale fixed** from user screenshots: `modelScale 1.35→1.0`, `overviewModel.scale 0.5→0.7`, and adopted DOMINATOR's flat `track` (EDGE's old one had `rz` tilts + an `s:1.3` spike that threw the long bar diagonally off-frame).
- **`featureStops[].x` centred (0) on all products EXCEPT DOMINATOR** — this section's copy is hard-centred by CSS (`.featureseq__stop` is `left:50%` + `text-align:center`), so the model belongs on the same axis; the tour reads through rotation. **User explicitly asked that DOMINATOR not be touched at all** — its `-0.3/0/0.15` offsets and its `colorModels` were reverted and byte-compared against HEAD to confirm.
- **Removed the `/product/drift-classic` route** (`CLASSIC_ALIASES`). That made `ProductHero`/`ProductGallery`/`ProductActions` unreachable — every slug in `products.ts` also has a `PRODUCT_EXPERIENCES` entry, so the classic branch can never render them. Deleted those three + `ProductPlaceholder.tsx` (also had zero importers) and the now-dead `PRODUCT_IMAGE_COUNTS` export. The classic fallback itself was kept as a safety net for a Shopify handle with no experience.
- **Deleted 73 unused product images (449MB)** from `public/products/{slug}/{colour}/`. Kept 38: the first 1–2 per colour (they are the literal `colors[].images` fallbacks in `product-experience.ts`), plus home-page cart thumbnails and `/v5`–`/v8` demo-route images. An adversarial sub-agent tried six vectors to refute the unused claim (dynamic paths, prototype routes, `next.config` rewrites, CSS `url()`, prisma `dev.db`, git history, `.next` output) and could not. Verified after deletion: 23 unique local product-image requests across 12 routes, zero 404s.
- **Fixed a real 404 introduced by the GLB swap**: `RevolveShowcase.tsx` (home page) still pointed at the deleted `legend-model.glb`. Repointed to `Legend-Black.glb` after confirming the two tinted material names it recolours (`leather_red_02.001/.002`) still exist in the new export.
- Home-page mobile fixes: ParallaxGrid image was clipped to 0-height (`h-full` on the image `<Link>` collapses in the mobile single-column grid — made it `lg:h-full`), removed a permanent `scale-[1.3]` that cropped the speaker, made the spec list a compact label/value row on mobile, stacked RevolveShowcase vertically below `sm`, and retired the "Hear it for yourself" audio hint once the hero scrolls past (it was overlapping later sections' CTAs).

**Q&A / knowledge captured this session**
- **Where product images come from:** product pages + `/shop` are **Shopify CDN** (Shopify also overrides name, tagline, description, price, specs, and **colours**). The **home page is entirely local/hardcoded** (`/products/{slug}.png`, `/gallery/{slug}.png`, `/frames/`). All GLBs are always local — `merge-experience.ts` never overrides `model`/`colorModels`. Fallback chain per colour: Shopify colour image → product featured image → static hardcoded cutout.
