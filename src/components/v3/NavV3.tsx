    ...
    <>
      ...
      <header className="pointer-events-none sticky top-0 z-50 hidden md:block">
        <div className="mx-auto mt-4 max-w-5xl px-4">
          ...
          >
            ...
            <ul className="flex items-center gap-7 text-[0.78rem] font-medium uppercase tracking-[0.16em] text-[#0f0102]/70">
              <li><Link href="/v3" className="transition-colors hover:text-[#420206]">Home</Link></li>
              <li
                className="relative"
                onMouseEnter={() => setSpeakersOpen(true)}
                onMouseLeave={() => setSpeakersOpen(false)}
              >
                <Link href="/shop" className="flex items-center gap-1 transition-colors hover:text-[#420206]">
                  Speakers
                  ...
                  <div className="w-64 rounded-2xl border border-[rgba(66,2,6,0.12)] bg-[rgba(247,242,234,0.97)] p-2 shadow-xl backdrop-blur-xl">
                    {products.map((p) => (
                      <Link key={p.slug} href={`/product/${p.slug}`} className="flex items-center justify-between rounded-xl px-4 py-2.5 transition-colors hover:bg-[rgba(66,2,6,0.05)]">
                        <span className="font-display text-base tracking-wide text-[#0f0102]">{p.model}</span>
                        <span className="text-xs normal-case tracking-normal text-[#c8922a]">₹{p.price.toLocaleString("en-IN")}</span>
                      </Link>
                    ))}
                  </div>
                </div>
              </li>
              <li><Link href="/shop" className="transition-colors hover:text-[#420206]">Collection</Link></li>
              <li><Link href="/v3" className="transition-colors hover:text-[#420206]">About</Link></li>
            </ul>
      ...
      <header className={`sticky top-0 z-50 transition-all duration-500 md:hidden ${scrolled || menuOpen ? "border-b border-[rgba(66,2,6,0.12)] bg-[rgba(247,242,234,0.95)] backdrop-blur-xl" : "border-b border-transparent bg-transparent"}`}>
        <div className="flex h-16 items-center justify-between px-5">
          ...
          <button type="button" aria-label="Toggle menu" onClick={() => setMenuOpen((v) => !v)} className="relative z-50 flex h-10 w-10 flex-col items-center justify-center gap-[5px]">
            <span className={`block h-px w-6 bg-[#0f0102] transition-all duration-300 ${menuOpen ? "translate-y-[6px] rotate-45" : ""}`} />
            <span className={`block h-px w-6 bg-[#0f0102] transition-all duration-300 ${menuOpen ? "opacity-0" : ""}`} />
            <span className={`block h-px w-6 bg-[#0f0102] transition-all duration-300 ${menuOpen ? "-translate-y-[6px] -rotate-45" : ""}`} />
          </button>
        </div>
      </header>
      ...
      <div style={{ backgroundColor: "rgba(247,242,234,0.99)" }} className={`fixed inset-0 z-40 transition-all duration-500 md:hidden ${menuOpen ? "visible opacity-100" : "invisible opacity-0"}`}>
        <div className="flex h-full flex-col justify-center gap-8 px-8">
          <Link href="/v3" className="font-display text-4xl text-[#0f0102] transition-colors hover:text-[#420206]">Home</Link>
          <Link href="/shop" className="font-display text-4xl text-[#0f0102] transition-colors hover:text-[#420206]">Collection</Link>
          <div className="border-t border-[rgba(66,2,6,0.15)] pt-6">
            <p className="mb-4 font-pinyon text-2xl text-[#c8922a]">Speakers</p>
            <div className="flex flex-col gap-3">
              {products.map((p) => (
                <Link key={p.slug} href={`/product/${p.slug}`} className="flex items-center justify-between text-[#0f0102]/80 transition-colors hover:text-[#420206]">
                  <span className="font-display text-xl tracking-wide">{p.model}</span>
                  <span className="text-sm text-[#c8922a]">₹{p.price.toLocaleString("en-IN")}</span>
                </Link>
              ))}
            </div>
          </div>
